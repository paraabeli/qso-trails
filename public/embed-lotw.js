...
(()=>{
  ...
  async function refresh(){
    try{const r=await fetch('/api/public',{cache:'no-store'});if(!r.ok)return;desired=label(await r.json());if(el.textContent!==desired)el.textContent=desired;}catch{}
  }
  const observer=new MutationObserver(()=>{if(desired&&el.textContent!==desired)el.textContent=desired;});
